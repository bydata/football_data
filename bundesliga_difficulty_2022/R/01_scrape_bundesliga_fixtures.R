...
scrape_fixtures <- function(matchday) {
  base_url <- glue("https://www.dfb.de/bundesliga/spieltagtabelle/?spieledb_path=%2Fcompetitions%2F12%2Fseasons%2Fcurrent%2Fmatchday%2F{matchday}")
  
  # regex to extract the date from the date column
  # (extracts the first occurence)
  date_regex <- "[0-3][0-9]\\.[0-1][0-9]\\.202[01]"
...
# save output
write_rds(fixtures, "output/bundesliga_fixtures.RData")
write_csv(fixtures, "output/bundesliga_fixtures.csv")
