...
df_plot %>% 
  ...
  annotate(
    "label",
    label = c("21 Saisons", "34 Saisons"),
    x = max(season_2023$played) + 0.1,
    y = c(16, 10),
    hjust = 0, family = "Roboto Condensed", color = "grey90", size = 2.5,
    label.size = 0, fill = alpha("grey24", 0.8)
  ) +
  scale_x_continuous(limits = c(1, 8.25), breaks = seq(1, 8, 1)) +
  scale_y_continuous(position = "left", breaks = seq(0, 102, 3)) +
  coord_cartesian(clip = "off") +
  labs(
    title = "Mittelmäßiger Saisonstart von Borussia Dortmund",
    subtitle = "Borussia Dortmund erreicht nach 8 Spieltagen der
    <b style='color:#FDE100'>Saison 2022/'23</b> 15 Punkte",
...
ggsave(here(base_path, "bvb-saisonstart-lines-until-current-matchday.png"), dpi = 500, width = 8, height = 6.5)
...
ggsave(here(base_path, "bvb-saisonstart-lines-final_result.png"), width = 8, height = 4)
...
ggsave(here(base_path, "bvb-saisonstart-lines-final_result.png"), width = 7, height = 4)
