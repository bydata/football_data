...
scrape_fixtures <- function(matchday) {
  base_url <- glue("https://www.dfb.de/bundesliga/spieltagtabelle/?spieledb_path=%2Fde%2Fcompetitions%2Fbundesliga%2Fseasons%2F2022-23%2Fmatchday%2F{matchday}")
  
  # regex to extract the date from the date column
  # (extracts the first occurence)
  date_regex <- "[0-3][0-9]\\.[0-1][0-9]\\.202[23]"
...
# save output
write_rds(fixtures, "bundesliga_difficulty_2023/input/bundesliga_fixtures.rds")
write_csv(fixtures, "bundesliga_difficulty_2023/input/bundesliga_fixtures.csv")
